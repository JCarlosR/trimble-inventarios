...
@section('content')
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <h2>Listado de niveles del anaquel {{$place->name}}</h2>
                ...
                <div class="x_content table-responsive">
                    ...
                    <div class="col-md-9 form-inline">
                        <div class="col-md-8 input-group margen">
                            <span class="input-group-addon">Nivel</span><input type="text" id="search" class="form-control" placeholder="Búsqueda personalizada ...">
                        </div>
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach($levels as $level)
                            <tr>
                                <td>{{ $level->name }}</td>
                                <td>{{ str_limit($level->comment, $limit = 50, $end = '...') }}</td>
                                <td>
                                    <a href="{{url('/caja/'.$level->id.'/'.$shelf.'/'.$local)}}" class="btn btn-primary"><i class="fa fa-eye"></i> Cajas</a>

                                    <button type="submit" class="btn btn-success" data-id="{{ $level->id }}" data-name="{{ $level->name }}"
                                            data-comment="{{ $level->comment }} ">
                                        <i class="fa fa-pencil"></i>Editar
                                    </button>

                                    <button type="submit" class="btn btn-danger"  data-delete="{{ $level->id }}" data-name="{{ $level->name }}">
        ...
        <div id="modalEliminar" class="modal fade in">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ url('nivel/eliminar/'.$shelf.'/'.$local) }}" method="POST">
                        <div class="modal-body">
...
@section('scripts')
    <script src="{{ asset('js/location/level.js')}}"></script>
